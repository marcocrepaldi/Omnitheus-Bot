      ...
      <main className="flex-1 p-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">Gestão de tenants da plataforma</p>
          </div>
        ...
        {showForm && (
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 mb-6">
            <h3 className="text-lg font-semibold text-white mb-5">Novo Cliente</h3>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-xs text-gray-400 mb-1 block">Nome da empresa</label>
                <input value={form.nome} onChange={e => setForm(f => ({ ...f, nome: e.target.value }))}
                  placeholder="Ex: Acme Seguros" className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2 text-white placeholder-gray-500 text-sm" />
              </div>
              <div>
                <label className="text-xs text-gray-400 mb-1 block">Slug (identificador único)</label>
                <input value={form.slug} onChange={e => setForm(f => ({ ...f, slug: e.target.value.toLowerCase().replace(/\s/g, '-') }))}
                  placeholder="Ex: acme" className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2 text-white placeholder-gray-500 text-sm" />
              </div>
              <div>
              ...
              <div>
                <label className="text-xs text-gray-400 mb-1 block">Nome do responsável</label>
                <input value={form.owner_nome} onChange={e => setForm(f => ({ ...f, owner_nome: e.target.value }))}
                  placeholder="Nome completo" className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2 text-white placeholder-gray-500 text-sm" />
              </div>
              <div>
                <label className="text-xs text-gray-400 mb-1 block">E-mail do responsável</label>
                <input value={form.owner_email} onChange={e => setForm(f => ({ ...f, owner_email: e.target.value }))}
                  type="email" placeholder="email@empresa.com" className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2 text-white placeholder-gray-500 text-sm" />
              </div>
              <div>
                ...
                <input value={form.owner_senha} onChange={e => setForm(f => ({ ...f, owner_senha: e.target.value }))}
                  type="password" placeholder="Senha provisória" className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2 text-white placeholder-gray-500 text-sm" />
              </div>
            </div>
        ...
        <div className="space-y-3">
          {tenants.length === 0 && <p className="text-gray-500 text-sm">Nenhum cliente cadastrado.</p>}
          {tenants.map(t => (
            <div key={t.id} className={`bg-gray-900 border rounded-xl p-5 ${t.ativo ? "border-gray-800" : "border-gray-700 opacity-60"}`}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="p-2.5 bg-gray-800 rounded-lg"><Building2 size={20} className="text-gray-300" /></div>
                  <div>
                    <div className="flex items-center gap-2">
                      <p className="text-white font-semibold">{t.nome}</p>
                      <span className="text-gray-500 text-xs">/{t.slug}</span>
                    ...
                    <div className="flex items-center gap-4 mt-1.5">
                      <span className="flex items-center gap-1 text-gray-400 text-xs"><Users size={12} /> {t.total_usuarios} usuário(s)</span>
                      <span className="flex items-center gap-1 text-gray-400 text-xs"><Bot size={12} /> {t.total_robos} robô(s)</span>
                      <span className="flex items-center gap-1 text-gray-400 text-xs"><Activity size={12} /> {t.total_execucoes} execução(ões)</span>
                    </div>
                  </div>
                </div>
                {t.id !== 1 && (
