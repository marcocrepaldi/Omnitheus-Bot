    ...
    <div className="flex">
      ...
      <main className="flex-1 p-8">
            ...
            <div className="space-y-4">
              ...
              <div>
                <input value={form.cron_expr} onChange={e => setForm(f => ({ ...f, cron_expr: e.target.value }))}
                  placeholder="Expressão Cron (ex: 0 8 * * *)" className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2 text-white placeholder-gray-500 text-sm" />
                <div className="flex flex-wrap gap-2 mt-2">
                  {EXEMPLOS.map(ex => (
                    <button key={ex.valor} onClick={() => setForm(f => ({ ...f, cron_expr: ex.valor }))}
                      className="bg-gray-800 hover:bg-gray-700 text-gray-300 text-xs px-3 py-1 rounded-lg border border-gray-700">
                      {ex.label}
                    </button>
                  ))}
                </div>
        ...
        <div className="space-y-3">
          {agendamentos.length === 0 && <p className="text-gray-500 text-sm">Nenhum agendamento configurado.</p>}
          {agendamentos.map(ag => (
            <div key={ag.id} className="bg-gray-900 border border-gray-800 rounded-xl p-5 flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="p-2 bg-purple-900 rounded-lg"><CalendarClock size={20} className="text-purple-400" /></div>
                <div>
                  <p className="text-white font-medium">{nomeRobo(ag.robo_id)}</p>
                  <p className="text-gray-400 text-sm font-mono">{ag.cron_expr}</p>
                  {ag.proximo_run && (
              ...
              <div className="flex items-center gap-4">
                <span className={`text-xs px-2 py-1 rounded-full ${ag.ativo ? "bg-emerald-900 text-emerald-300" : "bg-gray-800 text-gray-400"}`}>
                  {ag.ativo ? "Ativo" : "Inativo"}
                </span>
                <button onClick={() => deletar(ag.id)} className="text-gray-400 hover:text-red-400 transition-colors"><Trash2 size={16} /></button>
              </div>
            </div>
          ))}
        </div>
